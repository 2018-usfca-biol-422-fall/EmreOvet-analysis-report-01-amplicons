...
```
```{r exploring-datasets}
summary(as.factor(melted_phyloseq$age_unit))
```
```{r exploring-datasets}
table(melted_phyloseq$Phylum, melted_phyloseq$Abundance)
```
```{r exploring-datasets}
summary(as.factor(melted_phyloseq$longitude))
```
...
```{r Abundance-of-phyla-on-left-shift-key-and-right-shift-keys}
melted_phyloseq %>%
  filter(sample_source %in% c("Left_shift", "Right_shift")) %>%
  ggplot(aes(x = sample_source, y = Abundance, fill = Phylum)) +
  geom_col(position = position_dodge()) + 
    theme(axis.text = element_text(angle = 90, hjust = 1)) +
  ggtitle("Abundance of Phyla on Left Shift Key and Right Shift Key")

```

```{r Abundance-of-family-in-actinobacteria-family-found-in-left-and-right-shift-keys}
melted_phyloseq %>%
  filter(sample_source %in% c("Left_shift", "Right_shift")) %>%
...
```
```{r Abundance-of-genus-in-propopionibacteraiceae-family-found-in-left-and-right-shift-keys}
melted_phyloseq %>%
  filter(sample_source %in% c("Left_shift", "Right_shift")) %>%
...
```
```{r The-host-subject-identities-for-the-Propionibacteriaceae-family-found-from-left-and-right-shift-keys}
melted_phyloseq %>%
  filter(sample_source %in% c("Left_shift", "Right_shift")) %>%
  filter(Family %in% c("Propionibacteriaceae")) %>%
  ggplot(aes(x = sample_source, y = Abundance, fill = Genus)) +
