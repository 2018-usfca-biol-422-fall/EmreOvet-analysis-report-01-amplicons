...
melted_phyloseq$sample_source[melted_phyloseq$sample_source == "Left_Shift"] <- "Left_shift"
```
...
```
```{r The-host-subject-identities-for-the-Propionibacteriaceae-family-found-in-shift-keys}
melted_phyloseq %>%
  filter(sample_source %in% c("Left_shift", "Left_Shift", "Right_shift")) %>%
  filter(Family %in% c("Propionibacteriaceae")) %>%
